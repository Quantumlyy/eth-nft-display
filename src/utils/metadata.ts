...
export type MetadataParsingData<T = BaseOSMetadata | undefined> = [metadata: T, valid: boolean];
export type MetadataParsingReturn<T = BaseOSMetadata | undefined> = MetadataParsingData<T> | Promise<MetadataParsingData<T>>;
...
export function metadataAPI(uri: string, chainId: number, token: Token, shouldProxy: boolean): MetadataParsingReturn {
	// eslint-disable-next-line prettier/prettier
	[uri, shouldProxy] = quirkEthOSStorefront(uri, chainId, token, shouldProxy);
	[uri, shouldProxy] = quirkEthNifty(uri, chainId, token, shouldProxy);
